...
- **Three persistent ZFS+Lustre knobs (`primarycache=metadata`, `atime=off`, `obdfilter.brw_size=4`) decide whether file-backed-zpool Lustre on UMA is usable**: recover ~85% of the single-node loopback ceiling on bulk IO; without them, default config delivers 32Ã less on 64 KiB writes and 400Ã less on 4 KiB random IOPS. `primarycache=metadata` on the OST dataset is the single load-bearing knob. â [Lustre on UMA workstations](artifacts/training/lustre-on-uma-workstations/lustre-on-uma-workstations.md)
- **vLLM cold model load is loader-bound, not storage-bound: a one-line loader swap cuts it 18–36×.** Default safetensors loader ~106 s with one CPU core pinned and the NVMe near-idle (~1% of link bandwidth); fastsafetensors ~6 s; RunAI streamer ~3 s; all serve byte-identical output. → [vLLM cold load](artifacts/inference/vllm-cold-load-loader-bound/vllm-cold-load-loader-bound.md)
- **Checkpoint I/O on Lustre was client-bound at every point measured, write and read.** One writer 0.78 GB/s with the disk under 55% busy; two writers 1.32 GB/s (1.69×), landing on the independently measured concurrent ceiling with the disk still idle. %util and w_await alone cannot adjudicate client-vs-storage on ZFS-backed Lustre; the ZFS txg kstat plus per-thread CPU can. → [Lustre checkpoint storage](artifacts/training/lustre-checkpoint-storage/lustre-checkpoint-storage.md)
- **At workstation scale on default settings, storage is not the LLM-pipeline bottleneck at any stage**: across data prep, distillation, fine-tuning, eval, and serving the constraint lives in another layer (application, GPU memory bandwidth, single-thread CPU, or write concurrency), and at proof-of-concept scale the defaults are good enough that storage never enters the conversation. It flips to the bottleneck by misconfiguration or at scale. → [LLM pipeline on a workstation](artifacts/concepts/llm-pipeline-on-a-workstation/llm-pipeline-on-a-workstation.md)
...
- [Lustre on UMA workstations](artifacts/training/lustre-on-uma-workstations/lustre-on-uma-workstations.md). How to stand up minimum-viable distributed Lustre on 2 UMA workstations with stock single-NVMe layouts (no destructive partitioning), and which configuration knobs are load-bearing. Three-knob trio recovers ~85% of loopback ceiling; default is unusable. Cross-node Lustre is architecturally ~6Ã slower than NFSoRDMA on cached reads (RPC framing + LDLM + osd-zfs + ko2iblnd stack depth); distributed Lustre's win is concurrent multi-client where aggregate hits 60â85% of loopback. Six-obstacle build cascade documented; reproduce time compresses 5Ã with the recipe encoded.
- [Lustre checkpoint storage](artifacts/training/lustre-checkpoint-storage/lustre-checkpoint-storage.md). Single-node vs 2-node FSDP2 full-SFT checkpoint A/B on Lustre. Writes and restore reads are client-bound, not substrate-bound; a second writer scales aggregate 1.69× with the disk still idle. Ships the attribution method (ZFS txg kstat + per-thread CPU) for when %util is misleading.
- [vLLM cold load is loader-bound](artifacts/inference/vllm-cold-load-loader-bound/vllm-cold-load-loader-bound.md). Loader A/B/C on a fine-tuned Qwen3-8B: default ~106 s (single-threaded tensor materialization) vs 3–6 s streaming loaders, byte-identical output. Plus eval-traffic serving measurements (prefix cache 83% hit, KV cache peak 5.5% of budget, audit log 4 KB/s): no serving touch point is storage-constrained.
...
- **Want the whole pipeline walked end to end, with the real per-stage numbers** → [artifacts/concepts/llm-pipeline-on-a-workstation/llm-pipeline-on-a-workstation.md](artifacts/concepts/llm-pipeline-on-a-workstation/llm-pipeline-on-a-workstation.md): one end-to-end build (data prep → distillation → fine-tune → eval → serve) on the two-node lab, each stage with its real storage I/O and the layer the bottleneck actually lived in. The functional companion to the theory map above.
...
Documentation is licensed under [CC-BY-4.0](LICENSE); code (the infra scripts, the benchmark harness, and the reproduce kits) under [Apache-2.0](LICENSE-CODE). AIHomeLab is a personal project and is not affiliated with, sponsored by, or endorsed by any employer. See [LICENSE](LICENSE) for the full disclaimer.
