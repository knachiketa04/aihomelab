...
Storage shows up at distinct, predictable touch points across the LLM development pipeline. Most of the time it isn't the bottleneck — GPU or network is. But when storage *does* dominate, it dominates wall-clock more than the GPU does, and the gap between obvious choice and right choice runs to single-digit and low-double-digit multipliers across the touch points this lab has measured ([007](../../data-prep/spark-nvme-fio-baseline/spark-nvme-fio-baseline.md), [008](../../training/full-sft-storage-touchpoints/full-sft-storage-touchpoints.md)). The bill for getting it wrong is paid in **idle accelerator capacity** — the most expensive line item in any modern AI infrastructure budget.
...
- **Training** — storage dominates in four regimes: memory-constrained platforms, high-cadence checkpointing, multi-node with sync taxes, large-context fine-tuning.
- **Inference** — dominates at cold start (loader choice beats tier choice), KV cache offload, PD-disagg KV transfer, and audit-log volume.

**Where storage isn't the bottleneck** (the framing vendor content can't write honestly — but a practitioner can):

- **Data prep** — small datasets that fit in single-node RAM, CPU-heavy transforms, or rate-limited upstream feeds cap the pipeline before storage does.
- **Training** — full-precision training on dGPU clusters with healthy parallel FS: GPU is the bottleneck, the storage tier is over-provisioned by design, and most teams will never see storage in their flamegraph.
- **Inference** — steady-state serving on healthy infra with a hot model: GPU compute (per-token throughput) and network egress (response streaming) dominate; storage is quiet.
...
**For AI infrastructure engineers** planning AI workloads at production scale — on-prem, cloud, serious workstation labs. Not for ML researchers; not a tutorial on training. The framings target the decision tree an AI infra engineer walks when storage is in their portfolio: which tier for which phase, how to size each one, where the bottleneck actually lives, and how much it costs when it goes wrong.
...
- **Capacity (within-run + over-time).** The working set during an active pipeline run typically reaches 2–10× the final dataset size, because each transform-and-write step keeps prior intermediates for restart-on-failure. That's the within-run footprint. The longer-horizon problem is that **datasets compound** — web crawls grow, customer-log volumes expand, each pipeline version adds a generation, evals accumulate. Raw-zone footprint at the six-month mark is often several multiples of the launch volume on continuous-crawl ingest with no aggressive lifecycle pruning. Final-output capacity is what training plans against; pipeline-run capacity is what data prep plans against this quarter; cumulative-growth capacity is what the **AI infra engineer plans against this year**, and the third number is the one most often under-budgeted.
- **Cost-tier management.** At dataset scale, storage is one of the largest line items in an AI infrastructure budget — large enough that tier choice and lifecycle policy are first-class concerns, not afterthoughts. Object storage providers expose multiple tiers (hot / nearline / coldline / archive, or vendor equivalents) with order-of-magnitude price differences; lifecycle rules automate transitions (e.g., move intermediate zones to colder tiers after N days, expire failed-run artifacts after retention windows). Egress between regions or clouds is a separate cost line that can dwarf storage itself at PB scale, and is a real consideration when the training cluster and the data-prep object store live in different regions. Cost discipline at this stage usually pays back more than throughput tuning — higher impact, lower technical risk.
...
| # | Touch point | What is read | What is written | What dominates | Anchor |
|---|---|---|---|---|---|
| 1 | Raw acquire | upstream feed / partner / crawl | object store (raw zone) | network ingress + object-store write bandwidth | *descriptive only — not measured in this lab* |
| 2 | Cleaning + filtering | object store (raw) | object store (cleaned) | concurrent write throughput from distributed workers; CPU for filter logic | *descriptive only — not measured in this lab* |
| 3 | Enrichment | object store (cleaned) ± external model | object store (enriched) | CPU/GPU for the enrichment model; network if the model is remote | *descriptive only — not measured in this lab* |
| 4 | Format conversion | object store (enriched) | object store (formatted) | concurrent write throughput; some CPU for serialization | *descriptive only — not measured in this lab* |
| 5 | Tokenization / encoding | object store (formatted) | object store (tokenized) | CPU (often single-threaded per worker); aggregate throughput depends on worker count | *descriptive only — not measured in this lab* |
| 6 | Sharding | object store (tokenized) | object store (sharded) | aggregate write throughput; **metadata service for file-create rate** | *descriptive only — not measured in this lab* |
| 7 | Stage-to-training-tier | object store (sharded) | training-fast tier (parallel FS / local NVMe / FUSE mount) | egress bandwidth from object store; ingest rate of training-fast tier | *descriptive only — not measured in this lab* |
...
- **Throughput.** Sustained write rate during checkpoint save is the rate-determining step for cadence trade-offs. Burst-vs-sustained gaps exist in every storage tier (consumer NVMe SLC fall-off, enterprise NVMe steady-state limits, shared-FS write-cache exhaustion, cloud object store rate limits) — size against the sustained number for your tier, not the advertised burst — see [007](../../data-prep/spark-nvme-fio-baseline/spark-nvme-fio-baseline.md) for one decomposition of these gaps on a consumer Gen5 NVMe. At production scale, [NVIDIA's DGX SuperPOD H100 reference architecture](https://docs.nvidia.com/dgx-superpod/reference-architecture-scalable-infrastructure-h100/latest/storage-architecture.html) specifies a sustained single-node read floor of **40 GB/s** for the "Best" performance tier, with an aspirational target near **80 GB/s** matching maximum network capability for demanding workloads — both numbers delivered over InfiniBand RDMA per H100 node. Useful anchor numbers for planning against.
- **Distributed parallelism strategy.** The parallelism strategy the training team chooses determines the storage access pattern more than any other architectural decision:
  ...
  - **If you remember nothing else from this list:** bigger models use more complex strategies, and more complex strategies make checkpoint topology a first-class storage planning concern. Each strategy creates a different checkpoint shape and a different dataset read pattern; AI infra engineers sizing storage for pre-training-scale jobs should know which strategy is planned before specifying tiers.
...
- **Concurrency.** Page cache is the OS's scratch RAM that holds recently-read and recently-written file blocks. Every touch point on a node fights over the same pool — a heavy checkpoint write throws out the dataset pages the next epoch was counting on. This coupling is the most-mis-modeled aspect of training storage.
...
| # | Touch point | What is read | What is written | What dominates | Anchor |
|---|---|---|---|---|---|
| 1 | Dataset acquire | upstream registry / object store | local NVMe cache | network bandwidth — first-run only, cached on subsequent epochs | [008](../../training/full-sft-storage-touchpoints/full-sft-storage-touchpoints.md) |
| 2 | Dataset load + packing | NVMe (cached path) | RAM (packed sequences) | memory bandwidth — per-epoch | [008](../../training/full-sft-storage-touchpoints/full-sft-storage-touchpoints.md) |
| 3 | Model acquire | weights registry | local NVMe cache | network bandwidth (xet / S3 / GCS protocol) — first-run only | [008](../../training/full-sft-storage-touchpoints/full-sft-storage-touchpoints.md) |
| 4 | Model load into compute | NVMe → CPU/GPU memory | — | CPU tensor deserialization OR NVMe read depending on regime (page-cache-served / partial / cold-cache) | [008](../../training/full-sft-storage-touchpoints/full-sft-storage-touchpoints.md) |
| 5 | Checkpoint save | compute memory | local storage / shared FS (sharded + consolidated formats) | sustained write rate of the storage tier; the just-written shard often stays in page cache, which sets up TP7's behavior | [008](../../training/full-sft-storage-touchpoints/full-sft-storage-touchpoints.md) + [007](../../data-prep/spark-nvme-fio-baseline/spark-nvme-fio-baseline.md) |
| 6 | Checkpoint restore | NVMe (cold) | UMA / GPU memory | sequential read ceiling for the file format; loader pattern matters more than tier choice | [008](../../training/full-sft-storage-touchpoints/full-sft-storage-touchpoints.md) |
| 7 | Deploy-format export | NVMe (re-read DCP shard) | NVMe (consolidated safetensors) | page cache state of TP5's output — **6× wall-clock variance based on page cache state at the time of export** (lab's 8B-model worked example) | [008](../../training/full-sft-storage-touchpoints/full-sft-storage-touchpoints.md) |
...
- **Loader choice often dominates storage tier.** *If your model loads slowly, the storage tier is often not the problem.* The default safetensors loader spends most of cold-start time parsing tensors on a single CPU thread — a faster disk doesn't help that; a faster loader does. Loaders like RunAI Streamer, fastsafetensors, and tensorizer can recover significant cold-load time by parallelizing tensor deserialization or streaming weights directly to GPU memory. The loader-vs-tier decision is one of the most-mis-made choices in inference infrastructure planning. Some cloud platforms have begun shipping serving-specific FUSE adapters with caching tuned for this workload (e.g., GKE's Cloud Storage FUSE serving profile with Rapid Cache).
- **KV cache as a tiered hierarchy.** With growing context lengths and token reuse becoming common across use cases, KV cache itself is becoming a major working-set problem. Production deployments tier KV across GPU memory (fastest, smallest), CPU RAM (roughly an order of magnitude more capacity, with bandwidth an order of magnitude lower and tail latency worse than that — PCIe round-trips aren't free), and local NVMe (another order of magnitude more capacity, with reads landing in tens of microseconds rather than nanoseconds). An emerging fourth tier — external/remote KV stores like Mooncake Store and InfiniStore — handles the longest-tail working sets at frontier labs; most deployments stop at NVMe. Cache hit rate at each tier determines GPU utilization and per-request throughput.
...
- **PD-disaggregation transfer path.** When prefill and decode are split across pods, the KV from prefill must transit to decode. **This is fabric, not storage** — in production deployments the path is an RDMA-class network with user-space memory registration (UCX, NIXL, RoCEv2), not a file or object store. It appears in this storage-touch-point guide because it sits on the model's data path; flagged here so a storage-side reader doesn't go hunting for an underlying file layer. The path's bandwidth and memory-registration model become rate-determining for how far disaggregation scales — and create a class of failure modes (memory-registration backends, fabric mismatches) that don't exist in monolithic serving.
- **Audit log write rate.** Request/response logs retained at full fidelity can match or exceed the model weight footprint per million requests, depending on response length and retention policy — chatty long-form workloads blow past the model footprint; short-output APIs with aggressive retention may never come close. Most-frequently-mis-sized touch point in serving — capacity planning often assumes "small text" and lands on undersized log tiers that throttle live traffic.
...
1. **Model load (cold start)** — weights move from durable storage into compute memory at pod start. Loader choice (RunAI Streamer, fastsafetensors, tensorizer, vLLM default) often more determining than tier choice; default loaders can be CPU-bound on tensor deserialization, not storage-bound.
...
| # | Touch point | What is read | What is written | What dominates | Anchor |
|---|---|---|---|---|---|
| 1 | Model load (cold start) | durable storage (object / NVMe / shared FS) | — | **loader choice** more than storage tier; default loader is often CPU-bound on tensor deserialization. Once loaded, weights stay resident in compute memory. | *descriptive only — not measured publicly in this lab* |
| 2 | KV cache placement | — | GPU memory / CPU RAM / NVMe / external (tiered) | GPU memory bandwidth at top tier; cross-tier transfer rate when offloading. Grows live, per token. | *descriptive only — not measured in this lab* |
| 3 | Prefix / prompt cache | RAM / GPU memory | — | memory bandwidth on hit; recompute cost on miss. Bounded by TTL or capacity. | *descriptive only — not measured in this lab* |
| 4 | PD-disagg transfer (fabric, not storage) | KV from prefill pod | KV to decode pod | network bandwidth (RDMA-class fabric in production); memory-registration model. See the PD-disagg note above — listed here because it's on the model's data path, not because it's a storage operation. | *descriptive only — not measured publicly in this lab* |
| 5 | Serving artifact cache | container registry + NVMe | — | network egress (image pull) on first deploy; cached once per node thereafter. | *descriptive only — not measured in this lab* |
| 6 | Trace + audit log | — | NVMe / object store | sustained write rate; volume planning at fleet scale (write-through, no caching). | *descriptive only — not measured in this lab* |
| 7 | Model swap / hot reload | NVMe → GPU memory | — | same as TP1 plus request-drain constraints; behavior depends on swap policy + traffic shape. | *descriptive only — not measured in this lab* |
...
**Cost of getting it wrong.** Storage decisions have a non-symmetric cost shape. Get them right and storage stays invisible — that's the goal at every stage. Get them wrong and the bill is paid in **idle accelerator capacity**: GPUs stalling on dataset reads, training runs blocked by slow checkpoint writes, serving pods cold-starting one at a time while traffic queues, prefill pods waiting on KV transfers from disaggregated decode pods. And in the worst regime, slow becomes failure — outstanding-I/O queues overflow, checkpoint timeouts crash training jobs, serving pods OOM under KV pressure — turning a throughput tax into a missed deliverable. AI accelerator capacity is the single most expensive resource in modern production infrastructure budgets; even modest throughput losses from a mis-sized storage tier compound into substantial annual waste at the scales where AI is actually deployed. This asymmetry is why the framings throughout this guide — *cold vs warm*, *what dominates*, the per-stage honesty boxes — exist: to prevent the mis-sizing in the first place.
...
The touch-point map is interface-agnostic — the same touch points exist across cloud, on-prem, and lab. What changes per environment is the protocol the workload speaks to storage. The actual decision an AI infra engineer makes is which interface to expose for each tier in each stage:
