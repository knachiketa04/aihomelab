#!/usr/bin/env bash
# run-cold-restore-nfsordma-host2.sh — TP6 cold-cache restore measurement on host 2, rank 1.
#
# Companion to run-cold-restore-nfsordma-host1.sh. Same workload, opposite rank, same
# logical checkpoint directory (visible via NFSoRDMA mount). See host 1's header comment
# for the reproduction context.
#
# IMPORTANT: launch within ~10 sec of run-cold-restore-nfsordma-host1.sh on host 1.
...
NODE_RANK=1
RUN_DIR="${NFS_MOUNT_PATH}/${PRIOR_PHASE}"   # restore reads via NFSoRDMA mount
...
[ -e "${RUN_DIR}/checkpoints/LATEST" ] || {
  echo "ERROR: ${RUN_DIR}/checkpoints/LATEST not visible from this host's NFS mount." >&2
  echo "       Run run-multinode-nfsordma-host{1,2}.sh first." >&2
  exit 1
}
LATEST_TARGET="$(readlink "${RUN_DIR}/checkpoints/LATEST")"
echo "==> restoring from: $LATEST_TARGET (visible via NFSoRDMA mount)"
...
# --- Headline check ---
echo
echo "=== headline check (TP6 cold-cache restore over NFSoRDMA, host 2 view) ==="
...
echo
echo "This host's local NVMe should show ~zero reads in the iostat log during restore —"
echo "rank 1 reads its DCP shard via NFSoRDMA from host 1, NOT from local disk."

echo
echo "==> run-cold-restore-nfsordma-host2.sh complete on $NODE."
