#!/usr/bin/env bash
# run-multinode-rsync-host2.sh — multi-node training (training only) on host 2, rank 1.
#
# Companion to run-multinode-rsync-host1.sh. Same workload, opposite rank. See the
# host 1 script's header comment for the full reproduction context.
#
# IMPORTANT: launch this within ~10 sec of run-multinode-rsync-host1.sh on host 1.
# Rank 0 (host 1) is the rendezvous master; this script (rank 1) connects to it.
#
...
echo
echo "==> run-multinode-rsync-host2.sh (training only) complete on $NODE."
echo "    Logs: ${LOG_DIR}/{iostat,rss,training}-${PHASE}-${NODE}.log"
echo "    NEXT: wait for run-multinode-rsync-host1.sh to finish on host 1,"
echo "          then run sync-after-training.sh on host 1 (NOT this host)."
